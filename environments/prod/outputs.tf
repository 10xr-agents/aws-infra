...
output "application_urls" {
  ...
  value = {
    ...
    # Production URLs (recommended for users)
    primary_app_url = var.create_cloudflare_dns_records ? module.cloudflare[0].custom_dns_record_urls.prod-main.https_url : (
    ...
    primary_api_url = var.create_cloudflare_dns_records ? module.cloudflare[0].custom_dns_record_urls.prod-service.https_url : (
      var.create_global_accelerator ? "https://${module.global_accelerator[0].accelerator_dns_name}" : (
      local.acm_certificate_arn != "" ? "https://${module.networking.nlb_dns_name}" : "http://${module.ecs.alb_dns_name}"
    )
    )
