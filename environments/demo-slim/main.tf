...
# AWS Reachability Analyzer Configuration

# 1. Internet to ALB Path
resource "aws_ec2_network_insights_path" "internet_to_alb" {
  source      = "0.0.0.0/0"
  destination = aws_lb.main.arn
  protocol    = "tcp"

  destination_port = 443  # Assuming HTTPS, change to 80 if using HTTP
  source_ip        = "8.8.8.8"  # Example public IP, you can change this

  tags = {
    Name = "${var.project_name}-internet-to-alb-path"
  }
}

resource "aws_ec2_network_insights_analysis" "internet_to_alb" {
  network_insights_path_id = aws_ec2_network_insights_path.internet_to_alb.id
  ...
  filter_in_arns = [
    ...
    aws_security_group.ecs_sg.arn,
    aws_lb.main.arn
  ]

  tags = {
    Name = "${var.project_name}-internet-to-alb-analysis"
  }
}

# 2. Internet to Global Accelerator Path
resource "aws_ec2_network_insights_path" "internet_to_ga" {
  source      = "0.0.0.0/0"
  destination = aws_globalaccelerator_accelerator.main.id
  protocol    = "tcp"
  ...
  tags = {
    Name = "${var.project_name}-internet-to-ga-path"
  }
}

resource "aws_ec2_network_insights_analysis" "internet_to_ga" {
  network_insights_path_id = aws_ec2_network_insights_path.internet_to_ga.id

  tags = {
    Name = "${var.project_name}-internet-to-ga-analysis"
  }
}

# 3. Global Accelerator to ALB Path
resource "aws_ec2_network_insights_path" "ga_to_alb" {
  source      = aws_globalaccelerator_accelerator.main.id
  destination = aws_lb.main.arn
  protocol    = "tcp"
  ...
  tags = {
    Name = "${var.project_name}-ga-to-alb-path"
  }
}

resource "aws_ec2_network_insights_analysis" "ga_to_alb" {
  network_insights_path_id = aws_ec2_network_insights_path.ga_to_alb.id
  ...
  tags = {
    Name = "${var.project_name}-ga-to-alb-analysis"
  }
}
