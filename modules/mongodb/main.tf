...
resource "aws_route53_zone" "mongodb_public" {
  ...
  name = var.public_domain != "" ? var.public_domain : "mongodb.${var.cluster_name}.com"

  tags = merge(
    local.common_tags,
    {
      Name = "${var.cluster_name}-mongodb-public-zone"
      Type = "Public"
    }
  )
}

# Public DNS Records for MongoDB instances (using existing Route53 or public zone)
resource "aws_route53_record" "mongodb_public_primary" {
  count = var.create_public_dns_records ? 1 : 0

  zone_id = var.use_existing_public_zone_id != "" ? var.use_existing_public_zone_id : aws_route53_zone.mongodb_public[0].zone_id
  name    = "mongodb-primary"
  type    = "A"
  ttl     = 300
  records = [aws_instance.mongodb[0].public_ip]

  depends_on = [aws_instance.mongodb]
}

resource "aws_route53_record" "mongodb_public_secondary1" {
  count = var.create_public_dns_records && var.replica_count > 1 ? 1 : 0

  zone_id = var.use_existing_public_zone_id != "" ? var.use_existing_public_zone_id : aws_route53_zone.mongodb_public[0].zone_id
  name    = "mongodb-secondary1"
  type    = "A"
  ttl     = 300
  records = [aws_instance.mongodb[1].public_ip]

  depends_on = [aws_instance.mongodb]
}

resource "aws_route53_record" "mongodb_public_secondary2" {
  count = var.create_public_dns_records && var.replica_count > 2 ? 1 : 0

  zone_id = var.use_existing_public_zone_id != "" ? var.use_existing_public_zone_id : aws_route53_zone.mongodb_public[0].zone_id
  name    = "mongodb-secondary2"
  type    = "A"
  ttl     = 300
  records = [aws_instance.mongodb[2].public_ip]

  depends_on = [aws_instance.mongodb]
}

# Multi-value DNS record for the replica set (public)
resource "aws_route53_record" "mongodb_public_cluster" {
  for_each = var.create_public_dns_records ? {
    for idx, instance in aws_instance.mongodb : idx => instance.public_ip
  } : {}
...
# Update the local connection string to use public IPs when public DNS is enabled
locals {
  # Public connection string for external access
  public_connection_string = var.create_public_dns_records ? format(
    "mongodb://%s:%s@%s/%s?replicaSet=%s&authSource=admin",
    var.mongodb_admin_username,
    var.mongodb_admin_password,
    join(",", formatlist("%s:27017", aws_instance.mongodb[*].public_ip)),
    var.default_database,
    local.replica_set_name
  ) : ""

  # Use public DNS names if available
  public_dns_connection_string = var.create_public_dns_records ? format(
    "mongodb://%s:%s@%s/%s?replicaSet=%s&authSource=admin",
    var.mongodb_admin_username,
    var.mongodb_admin_password,
    "mongodb-cluster.${var.use_existing_public_zone_id != "" ? data.aws_route53_zone.existing_public[0].name : aws_route53_zone.mongodb_public[0].name}:27017",
    var.default_database,
    local.replica_set_name
  ) : ""
}

# Data source for existing public zone (if using existing zone)
data "aws_route53_zone" "existing_public" {
  count = var.use_existing_public_zone_id != "" ? 1 : 0
  zone_id = var.use_existing_public_zone_id
}

# Store public connection string in SSM
resource "aws_ssm_parameter" "mongodb_public_connection_string" {
  count = var.create_public_dns_records && var.store_connection_string_in_ssm ? 1 : 0

  name  = "/${var.environment}/${var.cluster_name}/mongodb/public-connection-string"
  type  = "SecureString"
  value = local.public_dns_connection_string != "" ? local.public_dns_connection_string : local.public_connection_string

  description = "MongoDB public connection string for ${var.cluster_name}"
  tags        = local.common_tags
