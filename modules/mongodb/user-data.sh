...
if [ "$(file -s $DATA_VOLUME_DEVICE)" = "$DATA_VOLUME_DEVICE: data" ]; then
    echo "Formatting data volume..."
    mkfs -t xfs $DATA_VOLUME_DEVICE
fi
...
# Mount the volume
mount $DATA_VOLUME_DEVICE /data/mongodb

# Add to fstab for persistent mounting
echo "$DATA_VOLUME_DEVICE /data/mongodb xfs defaults,nofail 0 2" >> /etc/fstab
...
#!/bin/bash
INSTANCE_ID=$(curl -s http://169.254.169.254/latest/meta-data/instance-id)
VOLUME_ID=$(aws ec2 describe-instances --instance-ids $INSTANCE_ID --query 'Reservations[0].Instances[0].BlockDeviceMappings[?DeviceName==`'$DATA_VOLUME_DEVICE'`].Ebs.VolumeId' --output text)
