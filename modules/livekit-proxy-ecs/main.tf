# modules/livekit-proxy-ecs/main.tf

/**
 * # LiveKit Proxy Service Module for ECS
 *
 * This module creates an ECS service for the LiveKit proxy service with:
 * - ECS Task Definition with LiveKit proxy container
 * - ECS Service with auto-scaling capabilities
 * - Target Group for load balancer integration
 * - Service Discovery for internal communication
 * - CloudWatch logs for monitoring
 * - Configuration matching the proxy service requirements
 */
...
# Task Definition for LiveKit Proxy
...
resource "aws_lb_target_group" "livekit_proxy" {
  name        = format("%.32s", "${var.cluster_name}-livekit-proxy-tg")
...
# Security Group for LiveKit Proxy
